...
export abstract class RestClient<T extends { id?: number }, F = null> {
  ...
  public async findAll (filters?: F): Promise<ResponseList<T>> {
    const response = await this.httpClient.get<ResponseListConstructor<T>>(this.path, {
      params: {
        ...filters
      }
    })
  ...
  public async findAllPeageble (filters?: F): Promise<ResponseListPeageble<T>> {
    const response = await this.httpClient.get<ResponseListPeagebleConstructor<T>>(this.path, {
      params: {
        ...filters
      }
    })
